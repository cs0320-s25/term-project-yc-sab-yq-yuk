...
public class RecommendationUtils {
    ...
    /**
     * Generate personal match scores for all events for the user.
     * Score is based on the user's like history, decayed over time (LAMDA),
     * and category overlap between liked events and current events.
     *
    ...
    public static Map<String, Double> computePersonalMatchScores(UserProfile user, List<Event> events) {
        ...
        Map<String, Double> categoryWeights = new HashMap<>();  // {category: total decayed weight from user's likes}
        ...
        for (UserProfile.LikeEntry like : user.getLikes()) {
            ...
            Event likedEvent = events.stream()
                    .filter(e -> e.getId().equals(like.getEventId()))
                    .findFirst()
                    .orElse(null);
            ...
            long daysSinceLike = Duration.between(
                    LocalDateTime.parse(like.getTimestamp()),
                    LocalDateTime.now()
            ).toDays();
            double decayWeight = Math.exp(-LAMBDA * daysSinceLike);  // exponential decay

            // Distribute decay weight equally among all categories of the liked event
            for (String category : likedEvent.getCategories()) {
                categoryWeights.put(category,
                    categoryWeights.getOrDefault(category, 0.0) + decayWeight / likedEvent.getCategories().size());
            }
        }
    ...
    /**
     * Computes the maximum popularity score among all events
     * Popularity is defined as the sum of an event's liked count and viewed count
     * This value is used to normalize trending scores
     *
    ...
    /**
     * Generates a random score to slightly boost a subset of events
    ...
    /**
     * A wrapper class that pairs an Event with its computed recommendation score
     * Used for sorting and ranking purposes
     */
