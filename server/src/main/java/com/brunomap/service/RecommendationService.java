...
/**
 * This is equivalent to logic inside a Spark handler
 * In this case, recommendation logic here it is!
 */
@Service
public class RecommendationService {
    ...
    /**
     * For user, recommend new events by scoring each event based on the 70/20/10 formula:
     * 70% personalization (with real-time decay) + 20% trending + 10% random,
     * and return top 50. (TODO: ideally add pagination here)
     * 
     * @param userId
    ...
    public List<Event> getRecommendations(String userId) throws Exception {
        ...
        /**
         * In the end, getting user will be replaced by /api/user/:user_id/profile
         * and getting events will be replaced by /api/events
         */
        ...
        if (user.getLikes().isEmpty()) {
            ...
            System.out.println("Welcome to BrunoMap! Generating fresh recommendations to get you started...");
        ...
        Map<String, Double> personalMatchScores = RecommendationUtils.computePersonalMatchScores(user, events);
     ...
     */
    private static List<Event> scoreAndSortEvents(List<Event> events, 
        ...
        double randomWeight) 
    {
        ...
        double maxPopularity = RecommendationUtils.computeMaxPopularity(events); // TODO: remove this once precalculation of trending score is added

        // Compute final score for each event based on the 70/20/10 formula
        List<ScoredEvent> scoredEvents = new ArrayList<>();
        for (Event event : events) {
            double personalScore = (personalMatchScores != null) ? personalMatchScores.getOrDefault(event.getId(), 0.0): 0.0;
            // TODO: pre-calculate trending scores periodically (e.g., hourly/daily) instead of on the fly here
            ...
            double randomScore = RecommendationUtils.computeRandomScores();  // random injection: 30% events get a random score boost of 0.2

            double finalScore = personalWeight * personalScore + trendingWeight * trendingScore + randomWeight * randomScore;
