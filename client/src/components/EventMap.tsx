...
export default function EventMap() {
  ...
  const filteredEvents = events.filter((event) => {
    ...
    // You would add more filter logic for time and location here
  ...
  // Trending filter
//   if (showTrending) {
//     // Use the trending_score property if available, otherwise use a calculated score
//     const trendingScore = event.trending_score || 
//       (event.liked_count * 2 + event.viewed_count) / 100;
      
//     // Only show events with high trending scores
//     if (trendingScore < 0.7) {
//       return false;
//     }
//   } else if (!selectedCategory && !timeFilter && !locationFilter && !searchQuery) {
//     // "For You" logic (personalized recommendations)
//     // This is a placeholder - in a real app, this would use user preferences, history, etc.
//     // For now, we'll just filter randomly as a demonstration
//     const userInterests = ["Academic Calendar", "Arts", "Faith"]; // Mock user interests
    
//     // Check if the event matches user interests
//     const matchesInterests = event.categories.some(category =>
//       userInterests.some(interest => category.includes(interest))
//     );
    
//     // Only show events that match interests or have high engagement
//     if (!matchesInterests && event.liked_count < 30 && !userLikes.includes(event.event_id)) {
//       return false;
//     }
//   }
              ...
              >
                ...
                {filteredEvents.map((event) => (
                  <Marker
                    ...
                    //color={getCategoryColor(event.categories)}
                    onClick={(e) => {
                ...
                {filteredEvents.length === 0 ? (
                  ...
                  filteredEvents.map((event) => (
                    <div
                      ...
                      // On click, fly to the event location and set it as selected
                      onClick={() => {
